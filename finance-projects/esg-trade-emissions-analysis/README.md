# Trade, Policy, and Carbon: Assessing Globalisation in Climate Agreements
...
The project is structured into three sequential Jupyter Notebooks to maintain clear separation of concerns between data exploration, causal inference, and model stability testing:
...
Across all baseline and interaction model layers, aggregate trade openness (trade_percent_gdp) shares no statistically significant relationship with the emissions gap. Broad globalisation metrics do not serve as reliable leading indicators or drivers of cross-border carbon leakage.
...
The absence of a broad trade-leakage effect implies that aggregate trade barriers are inefficient climate mitigation tools. Instead, climate governance frameworks should prioritise target-specific financial market instruments, such as the Carbon Border Adjustment Mechanism (CBAM), to directly address the carbon-pricing cost differentials that influence supply-chain routing.
